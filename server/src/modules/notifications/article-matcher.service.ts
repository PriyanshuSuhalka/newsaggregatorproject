...
export class ArticleMatcherService {
  private readonly logger = new Logger(ArticleMatcherService.name);

  // Category synonyms and related terms
  private readonly categorySynonyms: { [key: string]: string[] } = {
  ...
  private readonly commonStems: { [key: string]: string[] } = {
  ...
  /**
   * Enhanced matching algorithm that checks if an article matches user configuration
   */
  async matchArticle(article: Article, config: NotificationConfiguration): Promise<MatchResult> {
    ...
    if (categoryMatch.matched) {
      result.matched = true;
      result.score += categoryMatch.score;
      result.reasons.push(...categoryMatch.reasons);
      result.matchedCategories.push(...categoryMatch.matchedCategories);
    }
    ...
    if (keywordMatch.matched) {
      result.matched = true;
      result.score += keywordMatch.score;
      result.reasons.push(...keywordMatch.reasons);
      result.matchedKeywords.push(...keywordMatch.matchedKeywords);
    }

    // Bonus for multiple matches
    if (result.matchedCategories.length > 0 && result.matchedKeywords.length > 0) {
      result.score += 20; // Bonus for both category and keyword match
  ...
  private checkCategoryMatch(article: Article, config: NotificationConfiguration): MatchResult {
    ...
    for (const categoryId of config.enabledCategoryIds) {
      // We need to get the category name from the ID - this would require a database lookup
      // For now, we'll check against known category synonyms
