...
const thenewsapi_adapter_1 = require("./adapters/thenewsapi.adapter");
...
let ExternalApiService = ExternalApiService_1 = class ExternalApiService {
    constructor(articleRepo, categoryRepo, externalRepo, dataSource) {
    ...
    async fetchAndSaveArticles() {
        await this.fetchFromAdapter('newsapi', new newsapi_adapter_1.NewsApiAdapter(this.dataSource));
        await this.fetchFromAdapter('thenewsapi', new thenewsapi_adapter_1.TheNewsApiAdapter(this.dataSource));
    }
    async fetchFromAdapter(apiName, adapter) {
